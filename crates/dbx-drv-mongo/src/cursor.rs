...
use dbx_api::shape::{FieldDef, FieldFlags, LogicalType, Shape};
...
impl MongoCursor {
    ...
    /// dbx-core concern (§3.1) — this cursor only reports what's true about
    /// the wire-level documents it streamed.
    fn track_schema(&mut self, doc: &BsonDocument) -> Vec<dbx_api::shape::SchemaDelta> {
        ...
        for (k, v) in doc.iter() {
            ...
            deltas.push(dbx_api::shape::SchemaDelta::AddColumn {
                field: FieldDef {
                    name,
                    logical,
