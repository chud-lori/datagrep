...
use dbx_api::value::{FieldPath, PathSeg};
use dbx_api::DbError;
...
fn cmp_op(field: &FieldPath, op: &str, value: &dbx_api::Value) -> Result<BsonDocument, DbError> {
...
mod tests {
    use super::*;
    use dbx_api::value::Document as DbxDocument;
    use dbx_api::Value;
    use std::sync::Arc;
    ...
    fn ne_null_shaped_parameter_value_cannot_alter_query_structure() {
        let malicious = Value::Document(Arc::new(DbxDocument::from_fields(vec![(
            Arc::from("$ne"),
            Value::Null,
        )])));
        let pred = Predicate::Eq {
