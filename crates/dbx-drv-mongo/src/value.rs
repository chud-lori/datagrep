//! BSON <-> `dbx_api::Value` mapping (ticket item 4, design §3.1's "never
...
use dbx_api::{Document as DbxDocument, TzSpec, Value};

/// BSON `Decimal128` bit width, for [`unsupported_raw`]'s companion decode.
const OBJECT_ID_HEX_LEN: usize = 24;

/// Decode `bson::Bson` into `dbx_api::Value`, never failing and never losing
...
/// and the shape `dbx-lang`'s `ObjectId("...")` constructor accepts.
pub fn looks_like_object_id_hex(s: &str) -> bool {
    s.len() == OBJECT_ID_HEX_LEN && s.bytes().all(|b| b.is_ascii_hexdigit())
}

/// Encode `dbx_api::Value` to `bson::Bson` for predicate/mutation
...
pub fn value_to_bson(v: &Value) -> Result<Bson, dbx_api::DbError> {
    unsupported_err(match v {
        ...
        other => {
            return Err(dbx_api::DbError::Unsupported {
...
fn unsupported_err(b: Bson) -> Result<Bson, dbx_api::DbError> {
    Ok(b)
}

fn decode_unsupported(raw: &Bytes, type_name: &str) -> Result<Bson, dbx_api::DbError> {
    if raw.is_empty() {
        return Err(dbx_api::DbError::Unsupported {
            feature: format!(
...
fn value_doc_to_bson_doc(doc: &DbxDocument) -> Result<BsonDocument, dbx_api::DbError> {
...
/// report's "dbx-lang gap" note): `dbx-lang`'s `ObjectId("<hex>")`
...
pub fn value_to_bson_for_field(field: &str, v: &Value) -> Result<Bson, dbx_api::DbError> {
    if field == "_id" {
...
mod tests {
    ...
    fn missing_field_is_absent_not_null() {
        ...
        let dbx_doc = bson_doc_to_value_doc(&doc);
        assert_eq!(dbx_doc.get("nope"), None, "caller maps None to Absent");
    }
