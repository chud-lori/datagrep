//! `dbx-drv-mongo` — the MongoDB driver behind `dbx-api`'s `Driver` seam
//! (`/Users/nurchudlori/Projects/dbx-design.md` §3.1, §3.2, §3.3, §3.6, §5.1,
...
//!
//! - **The MongoShell parser is entirely `dbx-lang`'s.** This crate only
//!   consumes `dbx_lang::mongo::{parse, MongoStatement, ParsedMongo}`
...
//! - **No actor task for transactions**, unlike `dbx-drv-postgres`'s
...
//!    field names**, the same gap `dbx-drv-postgres` reports. For MongoDB
...
//!   `dbx-drv-postgres` wraps every mutation batch in one. On a standalone
...
//! - **dbx-lang's `ObjectId("...")` shell constructor compiles to
//!   `Value::Str(<hex>)`**, not `Value::Unsupported` (dbx-lang cannot depend
