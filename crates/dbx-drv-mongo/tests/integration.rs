...
//! All `#[ignore]`d by default — run with `cargo test -p dbx-drv-mongo --test
//! integration -- --ignored` against a server reachable at `DBX_TEST_MONGO`
...
//! *asserted on* goes through `dbx_api::Connection`/`Catalog`/`Canceller`,
...
use dbx_api::driver::{ConnectCtx, Connection, Driver, FetchHint, Payload};
use dbx_api::request::{Op, Request};
use dbx_api::shape::{ObjectPath, SchemaDelta};
use dbx_api::value::Value;

use dbx_drv_mongo::driver::MongoDriver;

fn test_uri() -> String {
    std::env::var("DBX_TEST_MONGO").unwrap_or_else(|_| "mongodb://localhost:27017".to_string())
}
...
fn unique_db(label: &str) -> String {
    ...
    let ts = SystemTime::now()
        ...
        .unwrap()
        .as_millis();
    format!("dbx_drv_mongo_test_{label}_{ts}_{n}")
}
...
async fn connect(database: &str) -> Arc<dyn Connection> {
    ...
    let mut cfg: ConnectionConfig = driver
        .parse_url(&test_uri())
        .expect("valid DBX_TEST_MONGO uri");
    cfg.values.insert(
        "database".to_string(),
    ...
    let conn = driver
        ...
        .await
        .expect("connect to test mongod (is DBX_TEST_MONGO reachable?)");
    Arc::from(conn)
}
