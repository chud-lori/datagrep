...
use datagrep_api::{FetchHint, Op, Payload, Request, ResumeToken, Value};

const KEY_COUNT: u32 = 50_000;
const HASH_FIELD_COUNT: u32 = 100_000;
const HASH_KEY: &str = "datagreptest:bighash";
const KEY_PREFIX: &str = "datagreptest:k:";
...
async fn hscan_pages_a_100k_field_hash_rather_than_returning_it_whole() {
    ...
    let mut cursor = conn
        .execute(Request::Op(Op::Scan {
            path: datagrep_api::ObjectPath::new(vec![
                std::sync::Arc::from("0"),
                std::sync::Arc::from("datagreptest:"),
