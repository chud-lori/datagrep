# datagrep-drv-redis integration tests
...
`common::flush`, which runs `FLUSHDB` against whatever `DATAGREP_TEST_REDIS`
...
```
export DATAGREP_TEST_REDIS=redis://localhost:6379   # optional; this is the default
cargo test -p datagrep-drv-redis -- --ignored --test-threads=1
```
...
```
cargo test -p datagrep-drv-redis --test scan_streaming -- --ignored
cargo test -p datagrep-drv-redis --test cancel -- --ignored
cargo test -p datagrep-drv-redis --test values_and_mutate -- --ignored
cargo test -p datagrep-drv-redis --test catalog -- --ignored
```
