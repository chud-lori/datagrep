...
//! constructed until first use" (`datagrep-core::api`), so registering every
//! compiled-in driver unconditionally costs nothing against the cold-start
//! budget — the `SqliteDriver`/`PostgresDriver` structs themselves aren't
//! built until a profile actually needs one.
...
/// Crate rule: no `if driver_id == …` branching above `datagrep-api`.
...
/// Stable registry ids this build knows about, for `datagrep doctor` and for
...
/// by `datagrep profiles add`/`show`/`doctor`). `CoreApi::register_driver` only
/// ever hands the registry a *constructor* (`datagrep_core::registry`: "nothing
...
/// Guess a profile's driver from a pasted connection URL (`datagrep profiles add`).
...
/// The SQL dialect (hence [`datagrep_lang::Language`]) a profile's driver speaks.
/// Static per driver id rather than read from a live connection's
/// `Capabilities`, so `datagrep query` can split/classify a script before
...
pub fn language_for_driver(id: &str) -> Option<datagrep_api::LanguageId> {
    match id {
        "sqlite" => Some(datagrep_api::LanguageId::Sql(
            datagrep_api::SqlDialect::Sqlite,
        )),
