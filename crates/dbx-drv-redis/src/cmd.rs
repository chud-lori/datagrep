...
use dbx_api::request::Predicate;
use dbx_api::value::Value;
use dbx_api::DbError;

/// Build a `redis::Cmd` from already-tokenized redis-cli-style arguments
/// (`dbx_lang::redis::tokenize_args`'s output). Arguments are sent as raw
...
/// named `key`, matching `dbx_api::value::FieldPath::field("key")`.
///
...
fn is_key_field(field: &dbx_api::value::FieldPath) -> bool {
    matches!(
        field.segments(),
        [dbx_api::value::PathSeg::Field(name)] if &**name == "key"
    )
}
