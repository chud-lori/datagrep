...
//!
//! TLS posture mirrors `dbx-drv-postgres`'s documented deviation: the ticket
...
/// dbx-api seam (`DbError` doc comment: "coarse by design").
pub fn map_redis_error(err: redis::RedisError) -> DbError {
    ...
    match err.kind() {
        ...
        ErrorKind::InvalidClientConfig => DbError::Config(dbx_api::ConfigError::InvalidValue {
            key: "url".into(),
