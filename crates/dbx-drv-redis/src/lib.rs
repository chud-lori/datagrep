//! # dbx-drv-redis
//!
//! Redis driver for `dbx`, implementing the frozen `dbx-api` seam
//! (`dbx-design.md` §3.1) over the [`redis`] crate's async
...
//! [`dbx_api::Enumeration::ScanOnly { requires_prefix: true }`] — design
...
//!   reads of your own writes, no savepoints), not what `dbx-api`'s
...
//!   [`dbx_api::driver::Enforcement::Client`] — Redis has no session mode
...
//!   dependency-list gap note (mirrors `dbx-drv-postgres`'s same deviation).
