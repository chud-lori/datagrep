//! RESP → [`dbx_api::Value`] mapping (design §3.1 requirement 5). Redis
...
//! (`dbx-api`'s "never lose bytes" rule).
//!
...
use dbx_api::value::{Document, Value};
use dbx_api::Bytes;

/// Map one RESP reply to a `dbx_api::Value`, recursively.
pub fn from_resp(v: redis::Value) -> Value {
    match v {
        ...
        redis::Value::Attribute { data, attributes } => {
            ...
            // RESP3 expiry hints); dbx-api's Value model has nowhere
            ...
            tracing::debug!(
                ...
                "dropping RESP3 attribute metadata (unsupported by dbx_api::Value)"
            );
            from_resp(*data)
        }
