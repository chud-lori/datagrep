...
//!
//! Three request doors, matching `dbx-api`'s `Request` enum:
//! - [`Request::Native`] — hand-typed redis-cli text. Split into one command
//!   per line with `dbx_lang::redis` (already built/tested — never
...
/// cloneable) does that; this type adds only what `dbx-api` requires on top
...
impl RedisConnection {
    ...
    /// Split `text` into one redis-cli command per line (`dbx_lang::redis`,
    ...
    async fn execute_native(
        ...
        let spans = dbx_lang::redis::REDIS.split(text);
        let mut commands: Vec<Vec<String>> = Vec::with_capacity(spans.len());
        for span in &spans {
            let line = span.text(text);
            let args = dbx_lang::redis::tokenize_args(line).map_err(|e| DbError::Query {
                code: None,
                message: e.to_string(),
                position: None,
            })?;
    ...
    fn native_scan_cursor(
        ...
        let resume = args
            .get(cursor_idx)
            .map(|c| ResumeToken(dbx_api::Bytes::from(c.clone().into_bytes())));
...
impl Connection for RedisConnection {
    ...
    /// `Enforcement::Client` is the honest answer: only `dbx-lang`'s
...
/// than forcing one (deviation noted in the crate report: `dbx-api` does
...
fn is_value_field(field: &dbx_api::FieldPath) -> bool {
...
mod tests {
    ...
    fn key_contains_colon_checks_str_and_bytes() {
        ...
        assert!(key_contains_colon(&Value::Bytes(
            dbx_api::Bytes::from_static(b"a:b")
        )));
    }

    #[test]
    fn is_value_field_matches_only_the_literal_value_field() {
        assert!(is_value_field(&dbx_api::FieldPath::field("value")));
        assert!(!is_value_field(&dbx_api::FieldPath::field("other")));
    }
}
