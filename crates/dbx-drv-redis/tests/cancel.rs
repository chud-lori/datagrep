...
//! *connection* must stay usable afterward (mirrors `dbx-drv-sqlite`'s own
//! cancellation test).
...
use dbx_api::{DbError, FetchHint, Op, Request};

const KEY_COUNT: u32 = 20_000;
const KEY_PREFIX: &str = "dbxtest:cancel:";
...
async fn cancel_stops_a_long_scan_loop_promptly_and_leaves_the_connection_usable() {
    ...
    let cancel_task = tokio::spawn(async move {
        ...
        assert_eq!(outcome, dbx_api::CancelOutcome::ClientAbandoned);
    });
    ...
    // dbx-drv-sqlite's cancel.rs — cancelling a cursor must never poison the
