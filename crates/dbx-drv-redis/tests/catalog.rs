...
use dbx_api::{Enumeration, ListOpts, ObjectKind, ObjectPath};
...
async fn children_walks_db_index_then_prefix_then_key() {
    ...
    common::seed_keys(&mut raw, "dbxtest:cat:", 25).await;
    ...
    assert!(
        prefixes
            .items
            .iter()
            .any(|n| n.path.parts().last().map(|s| &**s) == Some("dbxtest:")),
        "expected to find the dbxtest: prefix bucket, got {:?}",
        prefixes.items
    );
    ...
    let keys = catalog
        .children(
            &ObjectPath::new(vec![Arc::from("0"), Arc::from("dbxtest:cat:")]),
            key_opts,
        )
        .await
...
async fn describe_reports_type_and_ttl_for_a_real_key() {
    ...
    let detail = conn
        ...
        .describe(&ObjectPath::new(vec![
            Arc::from("0"),
            Arc::from("dbxtest:describe:"),
            Arc::from("dbxtest:describe:me"),
        ]))
        .await
