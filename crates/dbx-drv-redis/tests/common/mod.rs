...
//! - [`connect`] goes through the public `dbx-api` seam (`dbx_drv_redis::RedisDriver`)
//!   — every test exercises the driver exactly as any consumer above
//!   `dbx-api` would, no `pub(crate)` access.
...
use dbx_api::{ConnectCtx, Connection, Driver, ResolvedConfig};
use dbx_drv_redis::RedisDriver;

/// `DBX_TEST_REDIS`, defaulting to a local disposable instance.
#[allow(dead_code)]
pub fn test_url() -> String {
    std::env::var("DBX_TEST_REDIS").unwrap_or_else(|_| "redis://localhost:6379".to_string())
}
...
pub async fn connect() -> Box<dyn Connection> {
    ...
    let cfg = driver
        .parse_url(&test_url())
        .expect("parse DBX_TEST_REDIS url");
...
pub async fn raw_connection() -> redis::aio::ConnectionManager {
    let client = redis::Client::open(test_url()).expect("invalid DBX_TEST_REDIS url");
    client
...
/// this — `DBX_TEST_REDIS` must point at a disposable instance
