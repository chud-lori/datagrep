...
fn key_path(key: &str) -> ObjectPath {
    ObjectPath::new(vec![Arc::from("0"), Arc::from("dbxtest:"), Arc::from(key)])
}

async fn drain_pairs(cursor: &mut Box<dyn dbx_api::Cursor>) -> Vec<(Value, Value)> {
    let mut out = Vec::new();
...
async fn missing_key_maps_to_absent_not_null() {
    ...
    let mut cursor = conn
        .execute(Request::Op(Op::Scan {
            path: key_path("dbxtest:does-not-exist"),
            filter: None,
            order: Vec::new(),
            project: None,
            limit: None,
...
async fn string_hash_set_zset_list_all_fetch_type_aware() {
    ...
    // string
    let mut c = conn
        .execute(Request::native("SET dbxtest:s hello"))
        .await
        .expect("SET failed");
    while c
        .next_batch(FetchHint::default())
    ...
    for (f, v) in [("a", "1"), ("b", "2")] {
        let mut c = conn
            .execute(Request::native(format!("HSET dbxtest:h {f} {v}")))
            .await
            .expect("HSET failed");
        while c
            .next_batch(FetchHint::default())
    ...
    // set
    let mut c = conn
        .execute(Request::native("SADD dbxtest:set x y z"))
        .await
        .expect("SADD failed");
    while c
        .next_batch(FetchHint::default())
    ...
    // zset
    let mut c = conn
        .execute(Request::native("ZADD dbxtest:z 1 alice 2 bob"))
        .await
        .expect("ZADD failed");
    while c
        .next_batch(FetchHint::default())
    ...
    let mut c = conn
        .execute(Request::native("RPUSH dbxtest:l one two three"))
        .await
        .expect("RPUSH failed");
    while c
        .next_batch(FetchHint::default())
...
async fn op_count_dbsize_and_per_key_cardinality() {
    let mut raw = common::raw_connection().await;
    common::flush(&mut raw).await;
    common::seed_keys(&mut raw, "dbxtest:cnt:", 37).await;

    let conn = common::connect().await;
    let mut c = conn
        .execute(Request::native("HSET dbxtest:cnth a 1 b 2 c 3"))
        .await
        .expect("HSET failed");
    while c
        .next_batch(FetchHint::default())
...
async fn op_mutate_set_hset_del_are_atomic_and_report_native_counts() {
    ...
    let batch = MutationBatch {
        mutations: vec![
            Mutation::Insert {
                path: ObjectPath::new(vec![Arc::from("0"), Arc::from("dbxtest:mut:str")]),
                doc: Value::Str(Arc::from("hi")),
            },
            Mutation::Insert {
                path: ObjectPath::new(vec![Arc::from("0"), Arc::from("dbxtest:mut:h")]),
                doc: Value::Document(Arc::new(dbx_api::Document::from_fields(vec![(
                    Arc::from("f1"),
    ...
    let del_batch = MutationBatch {
        mutations: vec![
            Mutation::Delete {
                path: ObjectPath::new(vec![Arc::from("0")]),
                key: vec![Value::Str(Arc::from("dbxtest:mut:str"))],
            },
            Mutation::Delete {
                path: ObjectPath::new(vec![Arc::from("0")]),
                key: vec![Value::Str(Arc::from("dbxtest:mut:h"))],
            },
        ],
    };
    let mut cursor = conn
...
async fn native_multi_line_pipeline_dispatches_each_and_shapes_the_last() {
    ...
    let mut cursor = conn
        .execute(Request::native(
            "SET dbxtest:pipeline:a 1\nSET dbxtest:pipeline:b 2\nGET dbxtest:pipeline:a",
        ))
        .await
    ...
    let a: String = redis::cmd("GET")
        .arg("dbxtest:pipeline:a")
        .query_async(&mut raw)
        .await
        .expect("GET a failed");
    let b: String = redis::cmd("GET")
        .arg("dbxtest:pipeline:b")
        .query_async(&mut raw)
        .await
        .expect("GET b failed");
    assert_eq!(a, "1");
...
async fn native_scan_command_routes_through_the_paging_cursor() {
    let mut raw = common::raw_connection().await;
    common::flush(&mut raw).await;
    common::seed_keys(&mut raw, "dbxtest:native-scan:", 500).await;
    let conn = common::connect().await;

    let mut cursor = conn
        .execute(Request::native(
            "SCAN 0 MATCH dbxtest:native-scan:* COUNT 50",
        ))
        .await
