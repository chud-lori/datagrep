...
mod tests {
    ...
    fn maps_integer() {
        assert_eq!(
            sqlite_value_to_datagrep(ValueRef::Integer(42), None),
            Value::I64(42)
        );
        assert_eq!(
            sqlite_value_to_datagrep(ValueRef::Integer(-1), Some("INT")),
            Value::I64(-1)
        );
    }
    ...
    fn maps_integer_boolean_decl_to_bool() {
        assert_eq!(
            sqlite_value_to_datagrep(ValueRef::Integer(1), Some("BOOLEAN")),
            Value::Bool(true)
        );
        assert_eq!(
            sqlite_value_to_datagrep(ValueRef::Integer(0), Some("boolean")),
            Value::Bool(false)
        );
        // Boolean decl but a value outside {0,1}: honest INTEGER, not a lie.
        assert_eq!(
            sqlite_value_to_datagrep(ValueRef::Integer(7), Some("BOOLEAN")),
            Value::I64(7)
        );
    }
    ...
    fn maps_real() {
        assert_eq!(
            sqlite_value_to_datagrep(ValueRef::Real(3.5), None),
            Value::F64(3.5)
        );
    }
    ...
    fn maps_text() {
        assert_eq!(
            sqlite_value_to_datagrep(ValueRef::Text(b"hello"), None),
    ...
    fn maps_text_invalid_utf8_to_unsupported_without_losing_bytes() {
        ...
        let v = sqlite_value_to_datagrep(ValueRef::Text(raw), None);
        match v {
    ...
    fn maps_blob() {
        ...
        assert_eq!(
            sqlite_value_to_datagrep(ValueRef::Blob(bytes), None),
    ...
    fn date_time_decl_types_keep_storage_repr() {
        ...
        assert_eq!(
            sqlite_value_to_datagrep(ValueRef::Integer(1_700_000_000), Some("TIMESTAMP")),
