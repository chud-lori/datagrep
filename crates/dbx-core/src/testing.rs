...
//!
//! Sibling crates (`dbx-drv-*`, the spike UI) enable the `testing` feature to
...
use bytes::Bytes;
use dbx_api::caps::{Capabilities, Caps, LanguageId, ParamStyle, SqlDialect};
use dbx_api::catalog::{
    Catalog, Completion, CompletionCtx, Enumeration, InferredSchema, LevelDef, ListOpts,
    ObjectDetail, ObjectKind, ObjectNode, Page,
};
use dbx_api::config::{ConfigError, ConfigSchema, ConnectionConfig, ResolvedConfig};
use dbx_api::driver::{
...
use dbx_api::shape::{FieldDef, FieldFlags, LogicalType, ObjectPath, RowSchema, Shape};
...
pub fn mock_row_schema() -> RowSchema {
    ...
    RowSchema {
        ...
        identity: Some(dbx_api::shape::Identity {
            field_indices: vec![0],
        }),
    }
}
...
impl MockCursor {
    ...
    fn doc(&self, i: u64) -> Value {
        let card = self.plan.status_cardinality.clamp(1, STATUSES.len());
        Value::Document(Arc::new(dbx_api::value::Document::from_fields(vec![
