...
//!
//! > "**Invariant: `dbx` never holds a result set larger than
...
//! > window only**." (§3.2)
//!
//! [`dbx_query_rows`] is a single `CoreApi::get_rows(qid, off..off+len)` and a
...
//! back with `dbx_rows_pending() == true` and zero rows, which is the signal to
...
//! ```
//!
//! `dbx_rows_cell` returns a pointer that is either into `text` or **straight
...
use dbx_api::Value;
use dbx_core::store::{DocSegment, RowWindow, WindowSlice, WindowStatus};

use crate::cells::{render_arrow, render_value, Rendered, KIND_ABSENT};
use crate::ffi_util::{guard, guard_quiet, to_c_string};
use crate::query::{query_ref, DbxQuery};
use crate::runtime::runtime;

/// Where one cell's UTF-8 bytes live.
#[derive(Clone, Copy)]
struct CellMeta {
    /// Byte offset into [`DbxRows::text`] — used only when `ptr` is null.
    off: u32,
    len: u32,
    kind: u8,
...
impl std::fmt::Debug for DbxRows {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("DbxRows")
...
/// Borrow a `DbxRows*` argument.
///
/// # Safety
/// `r` must come from `dbx_query_rows` and not yet be freed.
unsafe fn rows_ref<'a>(r: *mut DbxRows) -> Option<&'a DbxRows> {
    if r.is_null() {
        None
    } else {
        Some(&*r)
...
/// The returned pointer must be freed with `dbx_rows_free` **before** its
/// `DbxQuery` is freed.
#[no_mangle]
pub unsafe extern "C" fn dbx_query_rows(
    q: *mut DbxQuery,
    offset: u64,
    len: u64,
    err_out: *mut *mut c_char,
) -> *mut DbxRows {
    guard(err_out, std::ptr::null_mut(), "dbx_query_rows", || {
        let q = query_ref(q)?;
        ...
        let Some(qid) = q.qid() else {
            return Ok(Box::into_raw(Box::new(DbxRows::skeleton(skeleton_cols))));
        };

        let rt = runtime()?;
        let window = rt
            .block_on(q.core().api.get_rows(qid, offset..offset.saturating_add(len)))
            .map_err(|e| e.to_string())?;
        Ok(Box::into_raw(Box::new(DbxRows::build(
            window,
            skeleton_cols,
        ))))
    })
...
impl DbxRows {
    ...
    fn build(window: RowWindow, fallback_cols: u32) -> Self {
        ...
        // the caller should draw skeletons for what `dbx_rows_count` does not
        // cover, so both set the flag (design §3.2 window resolver).
        let pending = matches!(
            window.status,
            WindowStatus::Pending | WindowStatus::Partial
        );
                        ...
                        for c in 0..cols {
                            let meta = match batch.columns().get(c) {
                                Some(array) => {
                                    let start = arena.len();
                                    let rendered =
                                        render_arrow(array.as_ref(), r, &mut arena);
...
/// [`crate::query::dbx_query_status_json`] to report a document result's
...
/// `r` must come from `dbx_query_rows` and not yet be freed.
#[no_mangle]
pub unsafe extern "C" fn dbx_rows_count(r: *mut DbxRows) -> u64 {
...
/// `r` must come from `dbx_query_rows` and not yet be freed.
#[no_mangle]
pub unsafe extern "C" fn dbx_rows_columns(r: *mut DbxRows) -> u32 {
...
/// `r` must come from `dbx_query_rows` and not yet be freed.
#[no_mangle]
pub unsafe extern "C" fn dbx_rows_pending(r: *mut DbxRows) -> bool {
    guard_quiet(false, || {
        rows_ref(r).map(|r| r.pending).unwrap_or(false)
    })
}
...
///
/// # Safety
/// `r` must come from `dbx_query_rows` and not yet be freed; `len_out` must be
/// NULL or point at a writable `size_t`. The returned pointer must not be used
/// after `dbx_rows_free`.
#[no_mangle]
pub unsafe extern "C" fn dbx_rows_cell(
    r: *mut DbxRows,
    row: u64,
    col: u32,
    len_out: *mut usize,
    ...
    guard_quiet(std::ptr::null(), || {
        ...
        if meta.len == 0 {
            ...
            // which the caller tells apart with `dbx_rows_cell_kind`. Return
...
/// `r` must come from `dbx_query_rows` and not yet be freed.
#[no_mangle]
pub unsafe extern "C" fn dbx_rows_cell_kind(r: *mut DbxRows, row: u64, col: u32) -> u8 {
...
/// `r` must come from `dbx_query_rows` and not yet be freed.
#[no_mangle]
pub unsafe extern "C" fn dbx_rows_cell_detail_json(
    r: *mut DbxRows,
    row: u64,
    col: u32,
) -> *mut c_char {
...
///
/// # Safety
/// `r` must come from `dbx_query_rows`, freed at most once. Every pointer
/// `dbx_rows_cell` returned for it is dangling afterwards.
#[no_mangle]
pub unsafe extern "C" fn dbx_rows_free(r: *mut DbxRows) {
    guard_quiet((), || {
        if !r.is_null() {
            drop(Box::from_raw(r));
        }
...
mod tests {
    ...
    fn docs_window(values: Vec<Value>) -> DbxRows {
        let len = values.len();
    ...
    fn text(rows: &DbxRows, row: u64, col: u32) -> String {
    ...
    fn absent_null_and_empty_survive_the_window() {
        let rows = docs_window(vec![
            ...
            doc(vec![("id", Value::I64(4)), ("note", Value::Str(Arc::from("")))]),
        ]);
    ...
    fn a_partial_window_delivers_what_exists_and_still_asks_for_more() {
        let rows = DbxRows::build(
            RowWindow {
                ...
                slices: vec![WindowSlice::Docs {
                    first_row: 0,
                    docs: Arc::new(DocSegment::Values(vec![doc(vec![(
                        "a",
                        Value::I64(1),
                    )])])),
                    offset: 0,
