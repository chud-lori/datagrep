...
//!   and freed by `dbx_string_free`, which is `CString::from_raw`. One
...
/// Allocate a NUL-terminated copy of `s` for the caller to `dbx_string_free`.
///
...
/// (`dbx_rows_count`, `dbx_rows_cell_kind`, the `free`s, …). A panic is
...
mod tests {
    ...
    fn null_err_out_is_tolerated() {
        let out = guard(std::ptr::null_mut(), 0i32, "test_fn", || {
            Err("nope".into())
        });
        assert_eq!(out, 0);
    }
