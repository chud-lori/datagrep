...
//! [`MongoError::UnsupportedJs`], the design doc's exact wording: *"dbx
...
impl<'s> Parser<'s> {
    ...
    /// escape hatch (dbx-api `value.rs`: "never lose bytes") is the more
    /// literal fit for a 12-byte BSON ObjectId, but its `raw` field is
    /// `bytes::Bytes`, a type dbx-api does not re-export — using it would
    /// require adding the `bytes` crate as a direct dependency, which this
    /// ticket restricts against (deps: dbx-api + thiserror only). Hex
    ...
    /// (dbx-api: "decimals stay strings" — never round-tripped through
    /// `f64`).
...
mod tests {
    ...
    fn rejects_for_loop_with_exact_message() {
        ...
        assert_eq!(
            err.to_string(),
            "dbx supports query expressions, not arbitrary JavaScript — use a raw command document for anything else"
        );
    }
