...
mod tests {
    ...
    async fn env_round_trip() {
        ...
        unsafe { std::env::remove_var("DATAGREP_SECRETS_TEST_ENV_ROUNDTRIP") };
    }

    #[tokio::test]
    async fn env_missing_var_errors() {
        unsafe { std::env::remove_var("DATAGREP_SECRETS_TEST_ENV_MISSING") };
        let r: SecretRef = "env:DATAGREP_SECRETS_TEST_ENV_MISSING".parse().unwrap();
    ...
    async fn keychain_live_round_trip() {
        ...
        let r: SecretRef = "keychain:datagrep-secrets-test:ci-round-trip"
            .parse()
            .unwrap();
        let res = resolver();
