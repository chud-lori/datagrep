...
//!
//! Connection profiles are plain-text, git-committable TOML that never
//! contain a secret — each secret field stores only a **reference**
//! ([`SecretRef`]), and the value is resolved just-in-time at pool creation.
//!
...
//!   (tens to hundreds of ms) and must never run on an async worker.
