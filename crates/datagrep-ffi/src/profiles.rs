...
pub unsafe extern "C" fn datagrep_profiles_list_json(
    ...
    guard(
        ...
        || {
            ...
            // Both UIs build their safety record from this payload; a key missing here is a guardrail silently off everywhere.
...
mod tests {
    ...
    unsafe fn take_json(p: *mut c_char, err: *mut c_char, what: &str) -> String {
        // SAFETY: p and err are NULL or strings the entry point under test allocated with to_c_string.
        unsafe {
            if !err.is_null() {
    ...
    unsafe fn expect_err(ok: bool, err: *mut c_char, what: &str) -> String {
        ...
        // SAFETY: non-NULL (asserted) and allocated by the entry point under test; datagrep_string_free matches.
        unsafe {
            let msg = CStr::from_ptr(err).to_string_lossy().into_owned();
    ...
    unsafe fn await_terminal(q: *mut crate::query::DatagrepQuery) -> String {
        for _ in 0..1500 {
            ...
            // SAFETY: q is the live handle from datagrep_query_run and is not freed until this returns.
            let status = unsafe {
                take_json(
