...
```sh
docker run --rm -d --name datagrep-pg-test -p 5432:5432 -e POSTGRES_HOST_AUTH_METHOD=trust postgres:16
```

Then run:

```sh
DATAGREP_TEST_PG=1 cargo test -p datagrep-drv-postgres --test integration -- --ignored --test-threads=1
```
...
`postgres`, no password/TLS. Override with `DATAGREP_TEST_PG_HOST`,
`DATAGREP_TEST_PG_PORT`, `DATAGREP_TEST_PG_USER`, `DATAGREP_TEST_PG_PASSWORD`,
`DATAGREP_TEST_PG_DB` if pointing at something else.

Tear down:

```sh
docker stop datagrep-pg-test
```
