//! Error type for `dbx-tunnel`.
//!
//! Security invariant, mirroring `dbx-secrets` (design §3.8): no variant's
//! `Display`/`Debug` may ever contain secret material (passphrases,
//! passwords, private key bytes). Those fields are always
//! `dbx_api::SecretString`, which redacts itself, or are simply not carried
