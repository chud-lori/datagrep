...
//!
//! ```text
//! DBX_TUNNEL_TEST_HOST=example.com \
//! DBX_TUNNEL_TEST_PORT=22 \
//! DBX_TUNNEL_TEST_USER=myuser \
//! cargo test -p dbx-tunnel --test live_ssh -- --ignored --nocapture
//! ```
//!
//! Auth defaults to `Auth::Agent` (`SSH_AUTH_SOCK`). Set
//! `DBX_TUNNEL_TEST_PASSWORD` to use password auth instead, or
//! `DBX_TUNNEL_TEST_KEYFILE` (optionally with
//! `DBX_TUNNEL_TEST_KEYFILE_PASSPHRASE`) for a key file.
//!
...
use dbx_tunnel::{Auth, SshTunnel, TofuStore};
...
fn test_auth() -> Auth {
    if let Some(password) = env("DBX_TUNNEL_TEST_PASSWORD") {
        return Auth::Password(SecretString::new(password));
    }
    if let Some(path) = env("DBX_TUNNEL_TEST_KEYFILE") {
        return Auth::KeyFile {
            path: path.into(),
            passphrase: env("DBX_TUNNEL_TEST_KEYFILE_PASSPHRASE").map(SecretString::new),
        };
    }
    Auth::Agent
}
...
async fn connect_and_open_channel_reads_a_real_ssh_banner() {
    let host = env("DBX_TUNNEL_TEST_HOST").expect("set DBX_TUNNEL_TEST_HOST");
    let port: u16 = env("DBX_TUNNEL_TEST_PORT")
        .map(|p| p.parse().expect("DBX_TUNNEL_TEST_PORT must be a u16"))
        .unwrap_or(22);
    let user = env("DBX_TUNNEL_TEST_USER").expect("set DBX_TUNNEL_TEST_USER");

    let known_hosts = std::env::temp_dir().join(format!(
        "dbx-tunnel-live-test-known-hosts-{}",
        std::process::id()
    ));
...
async fn reconnect_after_pinning_needs_no_prompt() {
    ...
    let known_hosts = std::env::temp_dir().join(format!(
        "dbx-tunnel-live-test-known-hosts-reconnect-{}",
        std::process::id()
    ));
