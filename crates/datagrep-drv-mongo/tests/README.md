# `datagrep-drv-mongo` integration tests

Unit tests (`src/*.rs`, `cargo test -p datagrep-drv-mongo --lib`) need no server:
...
cargo test -p datagrep-drv-mongo --test integration -- --ignored
```
...
```sh
DATAGREP_TEST_MONGO="mongodb://user:pass@host:27017" \
  cargo test -p datagrep-drv-mongo --test integration -- --ignored
```

Each test creates its own throwaway database (`datagrep_drv_mongo_test_<label>_
