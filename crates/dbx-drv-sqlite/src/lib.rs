//! # dbx-drv-sqlite
//!
//! SQLite driver for `dbx`, implementing the frozen `dbx-api` seam
//! (`dbx-design.md` §3.1) over [`rusqlite`].
//!
//! ## The concurrency shape (design §3.4)
//!
//! rusqlite is a synchronous, blocking API and a `rusqlite::Connection` is
//! not `Sync` (its statement handles borrow it). The design mandates that
//! sync drivers "wrap the whole cursor on a blocking thread with an mpsc
//! bridge — never `block_on` inside an async task." Concretely:
//!
...
//!   for its entire life. It is a plain OS thread, not `spawn_blocking` — the
//!   design calls this out explicitly because a DB client holds only a
//!   handful of connections (unlike a server pool), so paying for one real
//!   thread per connection is cheap and avoids statement handles crossing a
//!   pool.
...
//!   replies with a [`dbx_api::Batch`]. The cursor never touches SQLite
...
//!
//! ## Type honesty (design risk #4)
//! SQLite has type *affinity*, not enforced column types: a column declared
//! `INTEGER` can still store a `TEXT` value. `dbx-drv-sqlite` never guesses
//! past what a cell's actual SQLite storage class says — see
//! [`value::sqlite_value_to_dbx`] for the one exception (`BOOLEAN`-declared
