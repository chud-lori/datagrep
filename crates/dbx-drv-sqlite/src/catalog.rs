...
use crate::value::{quote_ident, sqlite_value_to_dbx, SqlParam};
...
/// primary key" (see the dbx-api gap noted in `driver.rs`).
...
/// `key: Vec<Value>` (see the dbx-api gap noted in `driver.rs`).
...
fn infer_shape_impl(
    ...
    while let Some(row) = rows.next().map_err(map_sqlite_err)? {
        ...
        for (i, trie) in tries.iter_mut().enumerate().take(col_count) {
            ...
            let value = sqlite_value_to_dbx(vref, None);
...
/// and dbx never translates user text; this only finds the prefix to look up).
