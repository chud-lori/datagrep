//! # dbx-secrets — secret references and resolution
//!
...
//! - Every resolved value is a [`dbx_api::SecretString`] — zeroized on drop,
//!   redacted from `Debug`.
...
/// compiler cannot elide the wipe. Mirrors `dbx_api::SecretString`'s drop
