...
//! and side-effect-free — `dbx --help` / `dbx profiles list` being
...
#[command(
    name = "dbx",
    version,
    about = "Same core, three faces: GUI, TUI, CLI. dbx query -f q.sql --format json | jq"
)]
pub struct Cli {
...
pub struct QueryArgs {
    /// Profile to run against (see `dbx profiles list`).
    #[arg(long)]
    pub profile: String,
    ...
    /// `-f`/`-c` is given, so `dbx query --profile p -` and piping with no
...
/// `dbx query --profile p -` matches the ticket's literal `-f file.sql | -c
...
mod tests {
    ...
    fn query_defaults_to_table_format() {
        let cli = Cli::parse_from(["dbx", "query", "--profile", "p", "-c", "select 1"]);
