...
//! statement and read its rows: [`dbx_core::CoreApi::run_query`] +
//! [`dbx_core::CoreApi::get_rows`], and `get_rows` answers out of
//! `dbx_core::store::ResultStore` — there is no lower-level façade method
...
//! describes, and that seam belongs in `dbx-core`, not here.
//!
...
//! [`dbx_core::CoreApi::subscribe_events`] already exists for exactly this,
//! so this function waits on it instead of re-polling `get_rows` in a spin.

#[cfg(test)]
use std::sync::atomic::{AtomicUsize, Ordering};

use dbx_core::store::{DocSegment, WindowSlice, WindowStatus};
...
fn doc_cell(v: &dbx_api::Value) -> CellText {
    match v {
        dbx_api::Value::Null => CellText::Null,
        dbx_api::Value::Absent => CellText::Absent,
