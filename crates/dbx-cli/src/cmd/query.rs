//! `dbx query` (ticket item 1): split input with `dbx-lang`, honor
...
//!
//! **CoreApi/driver gap:** `dbx_api::request::ExecOpts` declares
//! `row_limit`, `timeout`, and `read_only_assert` — but neither
//! `dbx-drv-postgres` nor `dbx-drv-sqlite` ever reads them (`grep` across
//! both crates for `row_limit`/`read_only_assert`/`ExecOpts` turns up
//! nothing but the struct literal that builds the request), and
//! `dbx-core` never inspects them either. So `--limit`/`@limit` and
...
//! purely client-side too, via `dbx_lang::Language::classify` — guardrail
...
use dbx_api::request::ExecOpts;
use dbx_api::Request;
use dbx_lang::StatementClass;
...
pub async fn run(ctx: &Context, args: &QueryArgs) -> Result<(), CliError> {
    ...
    let language = dbx_lang::language_for(dialect);
    ...
    for span in &spans {
        ...
        let status = match &run {
            Ok(outcome) if outcome.cancelled => dbx_profiles::HistoryStatus::Cancelled,
            Ok(_) => dbx_profiles::HistoryStatus::Ok,
            Err(_) => dbx_profiles::HistoryStatus::Error,
        };
        let _ = ctx
            .store
            .record_history(dbx_profiles::NewHistoryEntry {
                profile_id: profile.id.clone(),
                text: stmt_text.to_string(),
                started_at: dbx_profiles::now_ms(),
...
mod tests {
    ...
    async fn temp_sqlite_profile(store: &dbx_profiles::Store, name: &str, path: &std::path::Path) {
        let mut values = std::collections::BTreeMap::new();
        values.insert(
            "path".to_string(),
            dbx_api::ConfigValue::Str(path.display().to_string()),
        );
        let now = dbx_profiles::now_ms();
        let profile = dbx_profiles::Profile {
            id: dbx_profiles::new_id(),
            folder_id: None,
            name: name.to_string(),
            driver_id: "sqlite".to_string(),
            config: dbx_api::ConnectionConfig {
