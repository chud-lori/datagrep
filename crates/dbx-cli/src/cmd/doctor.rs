//! `dbx doctor` (ticket item 6): resolved config paths, registered drivers
...
pub async fn run(ctx: &Context, args: &DoctorArgs) -> Result<(), CliError> {
    ...
    for id in &registered {
        match crate::drivers::driver_for(id) {
            ...
            None => println!("  {id}: (registered, but not constructible from dbx-cli — bug)"),
        }
    }
    ...
    if let Some(name) = &args.profile {
        ...
        match &profile.secret_ref {
            None => println!("(none)"),
            Some(secret_ref) => match secret_ref.parse::<dbx_secrets::SecretRef>() {
...
fn describe_caps(flags: &dbx_api::Caps) -> String {
    use dbx_api::Caps;
...
mod tests {
    ...
    fn describe_caps_lists_known_flags() {
        let flags = dbx_api::Caps::TRANSACTIONS | dbx_api::Caps::DDL;
    ...
    fn describe_caps_handles_empty_flags() {
        assert_eq!(
            describe_caps(&dbx_api::Caps::empty()),
            "(no capability flags)"
        );
    }
}
