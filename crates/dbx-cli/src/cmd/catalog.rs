//! `dbx catalog` (ticket item 4): list ONE level lazily — exactly the
...
//!
//! `--describe` needs [`dbx_api::Catalog::describe`], which `CoreApi` does
...
//! `CoreApi`/`dbx-core` public API (`Session`, `ConnLease`, `Catalog` are all
//! `dbx-core`/`dbx-api` types), never a driver crate directly, but it skips
//! the panic-isolation wrapper `list_catalog` gets from `dbx_core::api`'s
...
/// positional `path` segments `dbx catalog` itself takes.
...
fn print_detail(detail: &dbx_api::catalog::ObjectDetail) {
    ...
    match &detail.schema {
        Some(schema) => {
            ...
            for field in &schema.fields {
                let nullable = if field.flags.contains(dbx_api::shape::FieldFlags::NULLABLE) {
                    "NULL"
                } else {
                    "NOT NULL"
                };
                let pk = if field
                    .flags
                    .contains(dbx_api::shape::FieldFlags::PRIMARY_KEY)
                {
                    " PK"
                } else {
                    ""
