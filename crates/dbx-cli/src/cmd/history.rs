//! `dbx history` (ticket item 5): the FTS5-backed history already living in
//! `dbx-profiles`. `dbx query` records one entry per statement it runs (see
...
fn print_entries(entries: &[dbx_profiles::HistoryEntry]) {
    if entries.is_empty() {
        println!("(no history)");
        return;
    }
