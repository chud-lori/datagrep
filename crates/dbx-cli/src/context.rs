...
//! hashmap insert per driver (`dbx_core::registry` docs: "nothing is
//! constructed until first use"), and `dbx_profiles::Store::open` is
//! documented lazy — its worker thread and SQLite file only come alive on
//! the first real call. This is what keeps `dbx --help` / `dbx profiles
...
pub struct Context {
    ...
    current_query: Mutex<Option<dbx_core::QueryId>>,
}
...
impl Context {
    ...
    /// which points at the developer's real `~/.config/dbx/profiles.db`.
    ...
    /// serializing tests or juggling a process-global `DBX_CONFIG_DIR`.
    ...
    pub fn set_current_query(&self, qid: Option<dbx_core::QueryId>) {
        *self
            .current_query
            .lock()
    ...
    pub fn current_query(&self) -> Option<dbx_core::QueryId> {
        *self
            .current_query
            .lock()
    ...
    pub async fn find_profile(&self, name: &str) -> Result<dbx_profiles::Profile, CliError> {
        ...
        profiles
            ...
            .ok_or_else(|| {
                CliError::usage(format!(
                    "no profile named `{name}` (see `dbx profiles list`)"
                ))
            })
    }
    ...
    /// build ever sees it (both `dbx-drv-postgres` and, if it grows a secret
    /// field, `dbx-drv-sqlite` fall back to reading the field straight out of
    /// `ConnectionConfig.values` when `ResolvedConfig.secrets` is empty). The
    /// on-disk profile never holds the secret (`dbx_profiles::secrets`
    ...
    pub async fn open_profile(
        &self,
        name: &str,
    ) -> Result<(dbx_core::ProfileId, dbx_profiles::Profile), CliError> {
        ...
        if let Some(secret_ref) = &profile.secret_ref {
            let reference: SecretRef = secret_ref
                .parse()
                .map_err(|e: dbx_secrets::SecretError| CliError::usage(e.to_string()))?;
        ...
        let core_profile = dbx_core::Profile {
            id: dbx_core::ProfileId(0), // overwritten by add_profile_full
...
fn map_env(env: dbx_profiles::Env) -> dbx_core::api::Env {
    match env {
