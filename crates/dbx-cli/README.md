...
The first runnable face of `dbx` — `CoreApi` over a terminal (design §4
...
`dbx --help` / `dbx profiles list` are near-instant (design P1, ≤250ms).

```
dbx query -f q.sql --format json | jq
```
...
shown is real, not illustrative. `$DBX` is the compiled binary
(`cargo build -p dbx-cli` → `target/debug/dbx`, or `target/release/dbx`).

```console
$ export DBX_CONFIG_DIR=$(mktemp -d)   # optional: isolates the profile store

$ $DBX profiles add local "sqlite:///tmp/app.db"
created profile `local` (sqlite)

$ $DBX query --profile local -c "
...
$ $DBX query --profile local -c "SELECT id, name, note FROM users ORDER BY id"
id  | name  | note
----+-------+----------
1   | alice | NULL
2   | bob   |
...
$ $DBX export --profile local -c "SELECT id, name, note FROM users ORDER BY id" \
...
|---|---|
| `dbx query --profile <name> [-f file.sql \| -c "SQL" \| -] [--format table\|json\|ndjson\|csv\|tsv] [--limit N] [--timeout 30s] [-o file]` | Splits input with `dbx-lang`, honors `-- @limit`/`@timeout`/`@connection`/`@readonly` directives, runs statements in order, streams every result window by window. |
| `dbx export --profile p -c "SQL" --format csv -o big.csv` | Same streaming path as `query`, but always to a file; rows/sec progress on stderr. |
| `dbx profiles list\|add\|remove\|show\|export\|import` | Plain-text, git-committable profiles. `add` splits an inline password out of the URL into a keychain `SecretRef`; `show` prints `secret: ••••`. |
| `dbx catalog --profile p [path...] [--describe path]` | Lists one level of the catalog lazily — one query per call, never a crawl. |
| `dbx history list\|search` | The FTS5-backed history in `dbx-profiles`; every statement `query` runs is recorded here. |
| `dbx doctor [--profile p]` | Resolved config paths, registered drivers with capabilities decoded, whether a profile's secret resolves, and a connection round-trip time. |
...
```console
$ $DBX query --profile local --format ndjson -o /tmp/bignum.ndjson -c \
...
a real frontend against it surfaced gaps worth fixing in `dbx-core`, in
...
1. **No store-free execution path, so `export` cannot honor design
   ...
   `dbx_core::store::ResultStore`. There is no lower-level façade (e.g. a
   raw `Cursor`/`Batch` stream) a frontend can drive directly. `dbx export`
   in this crate is therefore built on the identical path `dbx query` uses.
...
2. **`ExecOpts.row_limit`/`timeout`/`read_only_assert` are declared but never
   read.** `grep` across `dbx-drv-postgres` and `dbx-drv-sqlite` for
   `row_limit`/`read_only_assert`/`ExecOpts` turns up nothing but the struct
   literal that builds the `Request`; `dbx-core` never inspects them either.
...
3. **DDL/DML acknowledgement never reaches `CoreApi`.** `Shape::Ack {
   affected, message }` exists in `dbx-api`, and both drivers' `AckCursor`
   *carry* an affected-row count — but `AckCursor::next_batch` returns
   `Batch::default()` (`Payload::Empty`), and `dbx_core::store::convert()`
   ...
   1=0` — both show as "0 rows, no columns." `dbx query` says so explicitly
...
4. **No way to learn a `Shape::Table` result's columns when it has zero
   ...
   chunk (`ChunkBody::Table`); neither `dbx-drv-postgres` nor
   `dbx-drv-sqlite` ever populates `Batch::schema_delta` (`grep` confirms
   both always emit `Vec::new()`), and `StoreState.chunks` stays empty
   forever for a genuinely empty result. `dbx query` cannot print a header
...
- **No `[lib]` target.** Integration tests (`tests/cli.rs`) drive the
  compiled binary directly via `env!("CARGO_BIN_EXE_dbx")` rather than
...
- **`dbx catalog`'s laziness is proven behaviorally, not by an actual driver
  ...
  better proved inside `dbx-drv-sqlite`/`dbx-drv-postgres`'s own test
  suites, which can instrument the connection directly; this crate only has
  `CoreApi` to observe through.
- **The workspace does not build in place right now.** `crates/dbx-drv-redis`
  ...
  `dbx-cli` depends on, plus a root `Cargo.toml` mirroring the real one's
  ...
  `crates/dbx-cli/` itself was touched to work around this.
