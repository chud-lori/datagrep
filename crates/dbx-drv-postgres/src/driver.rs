...
/// exist on `dbx_api::Caps` (`crates/dbx-api/src/caps.rs` defines only the
/// ten below). Since `dbx-api` is the frozen seam this driver must not
...
mod tests {
    use super::*;
    use dbx_api::config::ConnectionConfig;
