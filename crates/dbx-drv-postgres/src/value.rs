...
use dbx_api::{shape::LogicalType, value::TzSpec, Value};
...
fn decode_array(elem_ty: &Type, raw: &[u8]) -> Value {
    ...
    for _ in 0..ndim {
        ...
        // lower bound (i32) — dbx has no concept of non-1 lower bounds; drop.
...
/// Wraps a `&dbx_api::Value` so it can implement the foreign `ToSql` trait
/// (orphan rules forbid implementing it directly on `dbx_api::Value`).
