...
//! default; run with `DBX_TEST_PG=1 cargo test -p dbx-drv-postgres --test
...
//! `postgres`, no password — override with `DBX_TEST_PG_HOST`,
...
use dbx_api::catalog::ListOpts;
use dbx_api::config::{ConfigValue, ConnectionConfig, ResolvedConfig};
use dbx_api::driver::{ConnectCtx, Connection, Driver, FetchHint};
use dbx_api::request::Request;
use dbx_api::shape::{ObjectPath, Shape};
use dbx_api::value::Value;

use dbx_drv_postgres::PostgresDriver;
...
async fn connect() -> Box<dyn Connection> {
    ...
    values.insert(
        "host".to_string(),
        ConfigValue::Str(env_or("DBX_TEST_PG_HOST", "localhost")),
    );
    values.insert(
        "port".to_string(),
        ConfigValue::Num(env_or("DBX_TEST_PG_PORT", "5432").parse().unwrap()),
    );
    values.insert(
        "user".to_string(),
        ConfigValue::Str(env_or("DBX_TEST_PG_USER", "postgres")),
    );
    values.insert(
        "database".to_string(),
        ConfigValue::Str(env_or("DBX_TEST_PG_DB", "postgres")),
    );
    values.insert("tls".to_string(), ConfigValue::Str("disable".to_string()));
    if let Ok(pw) = std::env::var("DBX_TEST_PG_PASSWORD") {
    ...
    driver.connect(&cfg, ConnectCtx::default()).await.expect(
        "connect to test postgres (set DBX_TEST_PG_* env vars if not on localhost:5432/postgres)",
    )
}
...
async fn streams_100k_rows_first_batch_arrives_fast() {
    ...
    let first_batch_rows = match &first_batch.payload {
        dbx_api::driver::Payload::Rows(rows) => rows.len(),
    ...
    while let Some(batch) = cursor
        ...
        .expect("batch")
    {
        if let dbx_api::driver::Payload::Rows(rows) = &batch.payload {
...
async fn streaming_does_not_retain_the_whole_result_set() {
    ...
    while let Some(batch) = cursor
        ...
        .expect("batch")
    {
        if let dbx_api::driver::Payload::Rows(r) = &batch.payload {
            rows += r.len() as u64;
        }
...
async fn numeric_round_trips_as_decimal_string() {
    ...
    let batch = cursor
        ...
        .expect("batch")
        .expect("one row");
    let dbx_api::driver::Payload::Rows(rows) = batch.payload else {
        panic!("expected rows")
    };
...
async fn cancel_mid_sleep_leaves_connection_usable() {
    ...
    assert_eq!(outcome, dbx_api::driver::CancelOutcome::Requested);
    assert!(
        saw_cancel_effect,
    ...
    let batch = cursor2
        ...
        .expect("one row");
    match batch.payload {
        dbx_api::driver::Payload::Rows(rows) => assert_eq!(rows[0][0], Value::I64(1)),
        other => panic!("expected Rows, got {other:?}"),
    }
}
...
async fn catalog_children_on_seeded_schema() {
    ...
    conn.execute(Request::native(
        "DROP SCHEMA IF EXISTS dbx_catalog_test CASCADE",
    ))
    .await
    .ok();
    conn.execute(Request::native("CREATE SCHEMA dbx_catalog_test"))
        .await
        .expect("create schema");
    conn.execute(Request::native(
        "CREATE TABLE dbx_catalog_test.widgets (id serial PRIMARY KEY, name text NOT NULL)",
    ))
    .await
    .expect("create table");
    ...
    assert!(
        schemas
            .items
            .iter()
            .any(|n| &*n.path.parts()[1] == "dbx_catalog_test"),
        "seeded schema should be listed"
    );

    let tables = catalog
        .children(
            &ObjectPath::new(vec![dbname.clone(), Arc::from("dbx_catalog_test")]),
            ListOpts {
                limit: 1000,
                ..Default::default()
            },
    ...
    conn.execute(Request::native("DROP SCHEMA dbx_catalog_test CASCADE"))
        .await
        .ok();
}
...
async fn scan_op_streams_with_identity() {
    let conn = connect().await;
    conn.execute(Request::native("DROP TABLE IF EXISTS dbx_scan_test"))
        .await
        .ok();
    conn.execute(Request::native(
        "CREATE TABLE dbx_scan_test (id serial PRIMARY KEY, v int NOT NULL)",
    ))
    .await
    .expect("create");
    conn.execute(Request::native(
        "INSERT INTO dbx_scan_test (v) SELECT * FROM generate_series(1, 10)",
    ))
    .await
    .expect("seed rows");

    let op = dbx_api::request::Op::Scan {
        path: ObjectPath::new(vec![Arc::from("dbx_scan_test")]),
        filter: None,
        order: vec![],
        project: None,
        limit: Some(5),
    ...
    conn.execute(Request::native("DROP TABLE dbx_scan_test"))
        .await
        .ok();
}
...
async fn non_select_returns_ack_shape_without_a_portal() {
    let conn = connect().await;
    conn.execute(Request::native("DROP TABLE IF EXISTS dbx_ack_test"))
        .await
        .ok();
    conn.execute(Request::native("CREATE TABLE dbx_ack_test (id int)"))
        .await
        .expect("create");
    let mut cursor = conn
        .execute(Request::native(
            "INSERT INTO dbx_ack_test VALUES (1), (2), (3)",
        ))
        .await
        .expect("insert");
    match cursor.shape() {
    ...
    assert!(cursor
        ...
        .unwrap()
        .is_none());
    conn.execute(Request::native("DROP TABLE dbx_ack_test"))
        .await
        .ok();
}
...
async fn quote_ident_survives_a_hostile_identifier() {
    ...
    conn.execute(Request::native(format!(
        ...
        dbx_drv_postgres::sql::quote_ident(hostile).unwrap()
    )))
    .await
    .ok();
    let create = format!(
