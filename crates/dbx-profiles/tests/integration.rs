//! Black-box tests against `dbx-profiles`'s public API only (design §3.7,
...
use dbx_api::{ConfigValue, ConnectionConfig};
use dbx_profiles::{
...
fn sample_profile(id: &str, folder_id: Option<String>) -> dbx_profiles::Profile {
    let now = now_ms();
    dbx_profiles::Profile {
        id: id.to_string(),
        folder_id,
        name: format!("profile-{id}"),
        driver_id: "postgres".to_string(),
        config: sample_config(),
        secret_ref: Some(format!("keychain:dbx:profile:{id}")),
        tunnel_id: None,
        env: Env::Dev,
...
fn sample_tunnel(id: &str) -> Tunnel {
    ...
    Tunnel {
        ...
        secret_ref: Some(format!("keychain:dbx:tunnel:{id}")),
...
async fn bak_snapshot_is_created_on_migration() {
    let dir = tempfile::tempdir().expect("tempdir");
    let path = dir.path().join("dbx.db");
    let bak = dir.path().join("dbx.db.bak");
...
async fn toml_export_then_import_round_trips_profiles() {
    ...
    assert!(exported.contains("keychain:dbx:profile:p1"));
...
async fn worker_thread_joins_cleanly_on_drop() {
    let dir = tempfile::tempdir().expect("tempdir");
    let store = Store::open(dir.path().join("dbx.db"));
