...
pub(crate) fn migrate(
    ...
    for (idx, migration) in MIGRATIONS.iter().enumerate().skip(current as usize) {
        ...
        tx.commit()?;
        tracing::info!(version, "dbx-profiles: applied migration");
    }
...
fn migrate_v1(tx: &Transaction<'_>) -> rusqlite::Result<()> {
    ...
    match tx.execute_batch(FTS5_SQL) {
        Ok(()) => tracing::debug!("dbx-profiles: FTS5 available, query_history_fts created"),
        Err(err) => tracing::warn!(
            %err,
            "dbx-profiles: FTS5 unavailable in this SQLite build; query_history search will use a LIKE fallback"
        ),
    }
    Ok(())
}
