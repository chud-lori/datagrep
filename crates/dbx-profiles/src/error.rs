...
//! `dbx_api::DbError`'s philosophy (design §3): callers match on a handful of
...
pub enum ProfilesError {
    ...
    #[error("failed to start dbx-profiles worker thread: {0}")]
    WorkerStart(String),

    /// The worker thread is gone (panicked or already shut down) so a
    /// request could not be completed.
    #[error("dbx-profiles worker thread is no longer running")]
    WorkerGone,
