/* DbxStub.c — a synthetic implementation of the frozen dbx C ABI.
 *
 * Purpose: let the macOS UI be built, run and MEASURED before crates/dbx-ffi
 ...
 *
 * Build with:  swift build                  (default; DBX_FFI unset or =stub)
 * Replace with the real static lib:  DBX_FFI=real swift build
 */
...
static void core_push(DbxCore *c, const char *name, const char *driver, const char *env,
                      int has_secret) {
    if (c->n == c->cap) {
...
DbxCore *dbx_core_new(const char *profiles_db_path, char **err_out) {
    ...
    DbxCore *c = (DbxCore *)calloc(1, sizeof(DbxCore));
    if (!c) {
...
void dbx_string_free(char *s) { free(s); }

char *dbx_profiles_list_json(DbxCore *c, char **err_out) {
    if (!c) {
...
static const StubProfile *find_profile(DbxCore *c, const char *name) {
    for (size_t i = 0; i < c->n; i++)
        if (strcmp(c->profiles[i].name, name) == 0) return &c->profiles[i];
    return NULL;
}

bool dbx_profiles_add(DbxCore *c, const char *name, const char *url, char **err_out) {
    if (!c || !name || !url) {
...
bool dbx_profiles_remove(DbxCore *c, const char *name, char **err_out) {
    if (!c || !name) {
        set_err(err_out, "null argument");
        return false;
    }
...
char *dbx_catalog_children_json(DbxCore *c, const char *profile, const char *path_json,
                                char **err_out) {
    if (!c || !profile) {
        set_err(err_out, "null argument");
        return NULL;
...
char *dbx_catalog_describe_json(DbxCore *c, const char *profile, const char *path_json,
                                char **err_out) {
    if (!c || !profile) {
        set_err(err_out, "null argument");
        return NULL;
...
DbxQuery *dbx_query_run(DbxCore *c, const char *profile, const char *sql, char **err_out) {
    if (!c || !profile || !sql) {
        set_err(err_out, "null argument");
        return NULL;
    }
    ...
    DbxQuery *q = (DbxQuery *)calloc(1, sizeof(DbxQuery));
    ...
    if (!sql_is_select(sql)) {
        /* Mirrors dbx-cli README gap #3: Shape::Ack never reaches the frontend,
...
void dbx_query_free(DbxQuery *q) {
    if (!q) return;
    pthread_mutex_lock(&q->lock);
    q->cancel_flag = 1;
...
void dbx_query_cancel(DbxQuery *q, char **outcome_json_out) {
    if (!q) return;
    pthread_mutex_lock(&q->lock);
...
char *dbx_query_status_json(DbxQuery *q, char **err_out) {
    if (!q) {
...
void dbx_query_on_progress(DbxQuery *q, DbxProgressFn cb, void *ctx) {
    if (!q) return;
    pthread_mutex_lock(&q->lock);
    q->cb = cb;
    q->cb_ctx = ctx;
...
DbxRows *dbx_query_rows(DbxQuery *q, uint64_t offset, uint64_t len, char **err_out) {
    if (!q) {
        set_err(err_out, "query is null");
        return NULL;
    }
    ...
    DbxRows *rw = (DbxRows *)calloc(1, sizeof(DbxRows));
    rw->count = count;
    rw->cols = ncols;
    rw->base_row = offset;
...
uint64_t dbx_rows_count(DbxRows *r) { return r ? r->count : 0; }
uint32_t dbx_rows_columns(DbxRows *r) { return r ? r->cols : 0; }
bool     dbx_rows_pending(DbxRows *r) { return r ? (r->pending != 0) : false; }

const char *dbx_rows_cell(DbxRows *r, uint64_t row, uint32_t col, size_t *len_out) {
...
uint8_t dbx_rows_cell_kind(DbxRows *r, uint64_t row, uint32_t col) {
    if (!r || row >= r->count || col >= r->cols) return 2;
    return r->kind[(size_t)row * r->cols + col];
}

char *dbx_rows_cell_detail_json(DbxRows *r, uint64_t row, uint32_t col) {
    if (!r || row >= r->count || col >= r->cols) return NULL;
    ...
    if (col == 11) {
        ...
        size_t l = 0;
        const char *p = dbx_rows_cell(r, row, col, &l);
        sb_put(&s, "\"");
