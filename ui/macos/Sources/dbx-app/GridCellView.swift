...
final class GridCellView: NSView {
    ...
    func setShimmer(_ on: Bool) {
        guard isPending else {
            layer?.removeAnimation(forKey: "dbx.shimmer")
            return
        }
        if on {
            guard layer?.animation(forKey: "dbx.shimmer") == nil else { return }
            wantsLayer = true
            let a = CABasicAnimation(keyPath: "opacity")
            a.fromValue = 0.45
            a.toValue = 0.95
            ...
            layer?.add(a, forKey: "dbx.shimmer")
        } else {
            layer?.removeAnimation(forKey: "dbx.shimmer")
        }
    }
