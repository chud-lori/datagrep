...
/// `DbxProgressFn` callback lands. If the block declared `-- @limit N` it is a
    ...
    var body: some View {
        if model.activeProfile.isEmpty {
            ContentUnavailableView {
                ...
                Text("Add a database and dbx will list its schema one level at a time — it never crawls.")
            } actions: {
