...
private struct DetailArea: View {
    ...
    var body: some View {
        ...
        .navigationTitle(model.activeProfile.isEmpty ? "dbx" : model.activeProfile)
    ...
    var body: some ToolbarContent {
        ToolbarItemGroup(placement: .navigation) {
            ...
            Menu {
                ...
                .disabled(model.activeProfile.isEmpty)
                .help(
                    "Client-side marker only — this ABI cannot set a profile's env, so dbx cannot learn it from the engine."
                )
            } label: {
        ...
        ToolbarItemGroup(placement: .primaryAction) {
            if model.hasDerivedClauses {
                ...
                .help("Remove the ORDER BY / WHERE dbx added by clicking headers and cells")
            }
...
/// `dbx_profiles_add` parses the URL and puts any inline password in the
