...
/// Owns one `DbxRows*`. Freed exactly once, in `deinit` — which is what makes
...
public final class RowWindow {
    ...
    init(raw: OpaquePointer, offset: UInt64) {
        self.raw = raw
        self.offset = offset
        self.count = dbx_rows_count(raw)
        self.columns = dbx_rows_columns(raw)
        self.pending = dbx_rows_pending(raw)
    }

    deinit { dbx_rows_free(raw) }
    ...
    public func kind(absoluteRow: UInt64, col: UInt32) -> CellKind {
        CellKind(raw: dbx_rows_cell_kind(raw, absoluteRow &- offset, col))
    }
    ...
    public func text(absoluteRow: UInt64, col: UInt32) -> String {
        var len = 0
        guard let p = dbx_rows_cell(raw, absoluteRow &- offset, col, &len), len > 0 else {
            return ""
        }
    ...
    public func detailJSON(absoluteRow: UInt64, col: UInt32) -> String? {
        takeOwnedString(dbx_rows_cell_detail_json(raw, absoluteRow &- offset, col))
    }
}
...
/// evicting a page drops its `DbxRows` immediately.
public final class RowPager {
    public let pageSize: UInt64
    public let maxPages: Int
    private unowned let query: DbxQueryHandle
    ...
    public init(query: DbxQueryHandle, pageSize: UInt64 = 512, maxPages: Int = 4) {
        self.query = query
    ...
    public func invalidateAll() {
        pages.removeAll()  // each RowWindow deinit -> dbx_rows_free
        order.removeAll()
    }
    ...
    public func window(for row: UInt64) -> RowWindow? {
        ...
        while order.count > maxPages {
            ...
            pages[victim] = nil  // dbx_rows_free happens here
            evictions += 1
        }
        return w.contains(row: row) ? w : nil
    }
