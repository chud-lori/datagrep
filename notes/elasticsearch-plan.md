...
| P1-5 | Index create/delete/close/open and alias add/remove as structured `DdlOp` | — | **Deferred to api M3, with the design settled and the reason measured.** The survey across all six drivers found that of these six operations exactly one — delete index — is a verb another engine also has. `Create` needs a type vocabulary the api does not have (`FieldDef` describes what a result *held*, and is deliberately lossy; authoring cannot be). `close`/`open` is an index lifecycle state no other engine has. And `POST /_aliases` is not "add an alias" but an **atomic action list** — a `remove`+`add` in one call is the zero-downtime swap, and `add` carries `filter` (a whole Query DSL document) and `routing`; a `DdlOp::AliasAdd { index, alias }` would drop all three, which `Native { text }` carries today. So the structured verbs worth adding are `Drop` and `Rename`, they need `ObjectKind` alongside the path (an index and an alias share one namespace here, and the server refuses `DELETE /<alias>`), and they wait for a caller: nothing in the workspace constructs `Op::Ddl` at all. See `DdlOp`'s own doc in `crates/datagrep-api/src/request.rs`. |
...
- Task-monitoring surface (`_tasks` polling) — we already have `_tasks` plumbing
  ...
  after it exists.
- Index templates — **read done**, in the root-level `describe` alongside cluster health. Not "one `GET`": there are **three** systems on the same cluster — composable `_index_template`, the `_component_template` pieces those are `composed_of`, and legacy `_template`, which still wins when no composable template matches. A stock 8.15 ships 45 / 44 / 5 of them before anyone authors one, so each is counted and its listing capped. The JSON APIs, not `_cat/templates`, which renders `index_patterns` as the string `"[logs-*]"` and does not know about component templates. Authoring a template is DDL and waits on the same gate as P1-5.
